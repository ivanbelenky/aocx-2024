defmodule DiskFragmenter do
  ...
  def consume_from_stack_2(stack, left_idx, right_idx) when left_idx >= right_idx,
    do: consume_from_stack_2(stack, 0, right_idx - 1)

  def consume_from_stack_2(stack, left_idx, right_idx) do
    IO.puts(right_idx)

    to_consume_and_index =
      Enum.filter(Enum.with_index(Enum.at(stack, right_idx)), fn {si, _} ->
        case si do
          {a, _} ->
    ...
    free_and_index =
      Enum.filter(Enum.with_index(at_left_idx), fn {si, _} ->
        case si do
          {:free, free_space} ->
            if free_space >= to_consume do
              true
    ...
    case Enum.at(free_and_index, 0) do
      {{:free, free_space}, free_idx} ->
        ...
        new_at_left_idx =
          List.insert_at(new_at_left_idx, free_idx + 1, {:free, free_space - to_consume})
      ...
      _ ->
        consume_from_stack_2(stack, left_idx + 1, right_idx)
    end
  end
