defmodule CeresSearch do
  ...
  def xmas_count(loc, i, j, _, _) do
    Enum.filter([@up, @down, @left, @right, @up_right, @up_left, @down_right, @down_left], fn path ->
      xmas = Enum.reduce(path, {[?X], i, j}, fn {di, dj}, {acc, ci, cj} ->
  ...
  def find_xmas(input) do
    ...
    list_of_charlists |> Enum.with_index() |> Enum.map(fn {line, idx_line} ->
      Enum.with_index(line) |> Enum.map(fn {char, idx_inline} ->
        case char do
    ...
    Enum.with_index(list_of_charlists) |>
      Enum.map(fn {line, j} ->
        Enum.with_index(line) |>
          Enum.map(fn {_, i} ->
            check_mask(list_of_charlists, i, j, @mas_x_mask_0) or check_mask(list_of_charlists, i, j, @mas_x_mask_1) or
            check_mask(list_of_charlists, i, j, @mas_x_mask_2) or check_mask(list_of_charlists, i, j, @mas_x_mask_3)
          end) |>
          Enum.filter(&(&1)) |>
          Enum.count()
      end) |>
