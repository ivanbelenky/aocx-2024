defmodule ResonantCollinearity do
  ...
  def all_antennas(mat) do
    Enum.reduce(Enum.with_index(mat), MapSet.new(), fn {row, j}, acc_all ->
      acc_local =
        Enum.reduce(Enum.with_index(row), MapSet.new(), fn {item, i}, acc_row ->
          case item do
            ?. -> acc_row
  ...
  def is_antinode(i, j, antenna_set, part \\ 1) do
    freq_to_pos =
      Enum.reduce(antenna_set, %{}, fn {x, y, freq}, acc ->
        Map.update(acc, freq, [{x, y}], fn existing -> [{x, y} | existing] end)
      end)

    antinode_check =
      Enum.map(freq_to_pos, fn {_, positions} ->
        n = length(positions)
  ...
  def find_antinodes(antinode_set, mat, i, j, _, antenna_set, part) do
    antinode_set =
      case is_antinode(i, j, antenna_set, part) do
        false -> antinode_set
