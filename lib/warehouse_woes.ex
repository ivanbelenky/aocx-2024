defmodule WarehouseWoes do
  ...
  def _step(pos, to_move, dir, box_positions, wall_positions, dr)
      when dir in [?>, ?<, ?^, ?v, ?O] do
    {dx, dy} = dr
    {i, j} = pos
    new_pos = {i + dx, j + dy}
    ...
    _step(
      new_pos,
      to_move,
      grid_val(new_pos, box_positions, wall_positions),
      box_positions,
      wall_positions,
      dr
    )
  ...
  def transition(start_pos, dir, box_positions, wall_positions) do
    ...
    cond do
      to_move == :nothing ->
        {start_pos, box_positions}

      length(to_move) == 1 ->
        {{x + dx, y + dy}, box_positions}
  ...
  def get_positions_of_char(grid, char) do
    Enum.reduce(Enum.with_index(grid), MapSet.new(), fn {row, j}, acc ->
      new_acc =
        Enum.reduce(Enum.with_index(row), MapSet.new(), fn {val, i}, val_acc ->
          case val == char do
  ...
  def _step2(pos, to_move, box_side, lbp, rbp, wp, {0, dy})
      when box_side in [?[, ?]] and dy in [-1, 1] do
    {i, j} = pos
    ...
    to_move =
      if Enum.member?(to_move, {lx, ly, :left}) do
        to_move
      else
        ...
        to_move_left =
          _step2(new_left, to_move, grid_val2(new_left, lbp, rbp, wp), lbp, rbp, wp, {0, dy})
    ...
    if Enum.member?(to_move, :nothing) do
      ...
      to_move =
        if Enum.member?(to_move, {rx, ry, :right}) do
          to_move
        else
          ...
          to_move_right =
            _step2(new_right, to_move, grid_val2(new_right, lbp, rbp, wp), lbp, rbp, wp, {0, dy})
  ...
  def transition2(start_pos, dir, lbp, rbp, wall_positions) do
    ...
    cond do
      ...
      length(to_move) > 1 ->
        ...
        {lefties, righties} =
          Enum.reduce(rest, {MapSet.new(), MapSet.new()}, fn r, {acc_l, acc_r} ->
            case r do
  ...
  def transform_to_grid({pos, lbp, rbp, wp}, {height, width}) do
    ...
    grid =
      Enum.reduce(lbp, grid, fn {i, j}, acc ->
        List.update_at(acc, j, fn row -> List.replace_at(row, i, ?[) end)
      end)

    grid =
      Enum.reduce(rbp, grid, fn {i, j}, acc ->
        List.update_at(acc, j, fn row -> List.replace_at(row, i, ?]) end)
      end)

    grid =
      Enum.reduce(wp, grid, fn {i, j}, acc ->
        List.update_at(acc, j, fn row -> List.replace_at(row, i, ?#) end)
      end)
